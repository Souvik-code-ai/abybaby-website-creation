...
export default function GridCell({ item, onClick }) {
  ...
  // Touch devices don't have a real ":hover" state. Framer's onHoverStart/
  // onHoverEnd are pointer-driven, so on mobile the *first* tap gets
  // consumed simulating "hover" (which mounts the overlay on top of the
  // tile) and only a *second* tap actually reaches onClick. That's why it
  // felt like you had to tap the little play badge specifically before the
  // reel would open individually — that badge sits above the overlay, so
  // it was the only spot still receiving the "real" click on the first tap.
  // Disabling the hover-driven overlay on touch devices makes the entire
  // tile a single, immediate tap target — same as clicking anywhere on
  // desktop.
  useEffect(() => {
