import toyotaCover from "../assets/images/drumtao/dt7.webp";
import toyotaDetail from "../assets/images/drumtao/dt4.webp";
import krishiCover from "../assets/images/arun/arun.webp";
import krishiDetail from "../assets/images/arun/arun3.webp";
import AbybabyDigital from "../assets/AbybabyDigital.mp4";
import pmsCover from "../assets/images/mia/mia2.webp";
import pmsDetail from "../assets/images/mia/mia3.webp";
import zira1 from "../assets/images/zira/zira1.webp";
import zira3 from "../assets/images/zira/zira3.webp";
import miaCover from "../assets/images/mia/mia3.webp";
import mallCover from "../assets/images/zira/zira1.webp";
import rallyCover from "../assets/images/drumtao/dt6.webp";
import tataCover from "../assets/images/arun/aarun1.webp";
import mia1 from "../assets/images/mia/mia 1.webp";
import mia2 from "../assets/images/mia/mia2.webp";
import mia3 from "../assets/images/mia/mia3.webp";
import mia4 from "../assets/images/mia/mia 2.webp";
import mall1 from "../assets/images/zira/zira2.webp";
import mall2 from "../assets/images/zira/zira3.webp";
import mall3 from "../assets/images/zira/zira4.webp";
import mall4 from "../assets/images/zira/zira1.webp";
import rally1 from "../assets/images/drumtao/dt2.webp";
import rally2 from "../assets/images/drumtao/dt3.webp";
import rally3 from "../assets/images/drumtao/dt4.webp";
import rally4 from "../assets/images/drumtao/dt5.webp";
import rally5 from "../assets/images/drumtao/dt6.webp";
import rally6 from "../assets/images/drumtao/dt7.webp";
import tata1 from "../assets/images/arun/arun2.webp";
import tata2 from "../assets/images/arun/arun3.webp";
import tata3 from "../assets/images/arun/arun4.webp";
import tata4 from "../assets/images/arun/arun5.webp";
import tata5 from "../assets/images/arun/arun6.webp";
import tata6 from "../assets/images/arun/arun7.webp";
import kangarookidsvdo1 from "../assets/images/kangarookids/kangarookidsvdo1.MP4";
import dp2 from "../assets/images/durgapuja/dp2.MP4";
import miavdo1 from "../assets/images/mia/miavdo1.MP4";
import miavdo2 from "../assets/images/mia/miavdo2.MP4";
import AbybabyIntro from "../assets/AbybabyIntro.mp4";
import dtvdo from "../assets/images/drumtao/dtvdo2.mp4";
import kangarooKidsLogo from "../assets/images/kangarookids/kangarookidslogo.webp";
import AbbLogo from "../assets/images/abb_logo.jpg";
import Logo from "../assets/images/logo.jpg";
import dt4 from "../assets/images/drumtao/dt4.webp";
export const STATS = [
  { label: "Clients", value: "350+" },
  { label: "Campaigns", value: "1200+" },
  { label: "Cities", value: "150+" },
  { label: "Consumers", value: "25M+" },
];
export const CAROUSEL_ITEMS = [
  {
    id: 1,
    title: "Annual Gala 2024",
    category: "Events",
    image: toyotaCover,
  },
  {
    id: 2,
    title: "Brand Activation",
    category: "Activation",
    image: krishiCover,
  },
  {
    id: 3,
    title: "Digital Campaign",
    category: "Digital",
    image: pmsCover,
  },
  {
    id: 4,
    title: "Exhibition Stand",
    category: "Exhibition",
    image: pmsDetail,
  },
  {
    id: 5,
    title: "Product Launch",
    category: "Events",
    image: zira3,
  },
];

export const SERVICES = [
  {
    id: 1,
    emoji: "🎯",
    title: "Brand Experiences",
    description:
      "Creating memorable experiences that connect brands with people.",
    services: [
      "Brand Activations",
      "Experiential Marketing",
      "Product Sampling",
      "Roadshows",
      "Consumer Engagement",
    ],
  },
  {
    id: 2,
    emoji: "🎪",
    title: "Events & MICE",
    description:
      "Delivering world-class corporate events and business experiences.",
    services: [
      "Corporate Events",
      "MICE & Conferences",
      "Dealer Meets",
      "Award Ceremonies",
      "Employee Engagement",
    ],
  },
  {
    id: 3,
    emoji: "🚀",
    title: "Product Launches & Exhibitions",
    description:
      "Launching products and showcasing brands through immersive spaces.",
    services: [
      "Product Launches",
      "Auto Launches",
      "Exhibition Stalls",
      "Pavilion Design",
      "Experience Zones",
    ],
  },
  {
    id: 4,
    emoji: "🎤",
    title: "Entertainment, Sports & Festivals",
    description:
      "Creating large-scale public experiences that inspire audiences.",
    services: [
      "Concerts & Entertainment",
      "Sports Events",
      "Festival Marketing",
      "Celebrity Management",
      "Public Engagement",
    ],
  },
  {
    id: 5,
    emoji: "🛠️",
    title: "Creative, Production & Fabrication",
    description: "From strategy to execution—we bring every idea to life.",
    services: [
      "Creative Strategy",
      "Campaign Design",
      "Production",
      "Fabrication",
      "Event Branding",
    ],
  },
  {
    id: 6,
    emoji: "💻",
    title: "Digital & Technology Solutions",
    description:
      "Helping brands grow through digital innovation and intelligent automation.",
    services: [
      "Digital Marketing",
      "Website Development",
      "Mobile Apps",
      "AI Chatbots & Automation",
      "Branding & Design",
    ],
  },
];

// ── Media data ────────────────────────────────────────────────────────────────

export const REELS_DATA = [
  {
    id: 101,
    type: "reel",
    src: kangarookidsvdo1,
    caption: "Behind the scenes — Product Launch setup timelapse.",
    likes: 1230,
    comments: 88,
    category: "Events",
    poster: kangarooKidsLogo,
  },
  {
    id: 102,
    type: "reel",
    src: dp2,
    caption: "Digital Campaign reel — 48 hours of content creation.",
    likes: 980,
    comments: 62,
    category: "Digital",
    poster: AbbLogo,
  },
  {
    id: 103,
    type: "reel",
    src: miavdo1,
    caption: "Annual Gala highlight reel — the moments that mattered.",
    likes: 2100,
    comments: 143,
    category: "Events",
    poster: mia1,
  },
  {
    id: 104,
    type: "reel",
    src: AbybabyDigital,
    caption: "Zero Festival vibes — energy like no other.",
    likes: 1750,
    comments: 109,
    category: "Festival",
    poster: Logo,
  },
  {
    id: 105,
    type: "reel",
    src: AbybabyIntro,
    caption: "Durga Puja reel — colours, lights and devotion.",
    likes: 3200,
    comments: 218,
    category: "Cultural",
    poster: Logo,
  },
  {
    id: 106,
    type: "reel",
    src: dtvdo,
    caption: "Heritage Dance Night — 60-second highlight.",
    likes: 890,
    comments: 55,
    category: "Cultural",
    poster: dt4,
  },
];

export const IMAGES_DATA = [
  {
    id: 1,
    type: "image",
    src: mia1,
    caption: "Mia By Tanishq",
    likes: 312,
    comments: 28,
    category: "Events",
  },
  {
    id: 2,
    type: "image",
    src: mia2,
    caption: "Zero Festival Opening Gala — lights, music and energy.",
    likes: 540,
    comments: 41,
    category: "Festival",
  },
  {
    id: 3,
    type: "image",
    src: mia3,
    caption: "Innovation Tech Conference — 720 leaders, one stage.",
    likes: 278,
    comments: 19,
    category: "Conference",
  },
  {
    id: 4,
    type: "image",
    src: mia4,
    caption: "EV SUV Grand Reveal — electric dreams become reality.",
    likes: 690,
    comments: 57,
    category: "Product Launch",
  },
  {
    id: 5,
    type: "image",
    src: tata1,
    caption: "Tata Bandhan Family Day — celebrating togetherness.",
    likes: 445,
    comments: 33,
    category: "Cultural",
  },
  {
    id: 6,
    type: "image",
    src: mall4,
    caption: "Durga Puja Celebration — colour, culture and community.",
    likes: 812,
    comments: 74,
    category: "Cultural",
  },
  {
    id: 7,
    type: "image",
    src: mall3,
    caption: "Heritage Dance Night — 320 guests moved by tradition.",
    likes: 234,
    comments: 16,
    category: "Cultural",
  },
  {
    id: 8,
    type: "image",
    src: mall1,
    caption: "Brand Activation in Bengaluru — live, loud and unforgettable.",
    likes: 367,
    comments: 24,
    category: "Activation",
  },
  {
    id: 9,
    type: "image",
    src: mall2,
    caption: "Exhibition stand for TechWorld 2024 — 3,200 sqft of wow.",
    likes: 198,
    comments: 11,
    category: "Exhibition",
  },
];
